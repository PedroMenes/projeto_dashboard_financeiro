...
$pageTitle = 'Nova Categoria — Controle Financeiro';
$db = getDB();

$erros = [];
$dados = ['nome' => '', 'tipo' => 'despesa'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        $chk = $db->prepare("SELECT id FROM categorias WHERE nome = ? AND tipo = ? LIMIT 1");
        $chk->execute([$dados['nome'], $dados['tipo']]);
        if ($chk->fetch())
            $erros[] = 'Já existe uma categoria com este nome e tipo.';
    }

    if (empty($erros)) {
        $stmt = $db->prepare("INSERT INTO categorias (nome, tipo) VALUES (?, ?)");
        $stmt->execute([$dados['nome'], $dados['tipo']]);
        header('Location: /projeto_dashboard_financeiro/categorias/index.php?msg=criada');
        exit;
    }
}
