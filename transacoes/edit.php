...
$pageTitle = 'Editar Transação — Controle Financeiro';
$user = currentUser();
$db   = getDB();

$id = (int)($_GET['id'] ?? 0);

// Busca a transação garantindo que pertence ao usuário logado
$stmt = $db->prepare("SELECT * FROM transacoes WHERE id = ? AND usuario_id = ?");
$stmt->execute([$id, $user['id']]);
$transacao = $stmt->fetch();

if (!$transacao) {
    header('Location: /projeto_dashboard_financeiro/transacoes/index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        ...
        header('Location: /projeto_dashboard_financeiro/transacoes/index.php?msg=editada');
        exit;
    }
}
...
<div class="container" style="max-width: 680px;">
    ...
    <div class="mb-4">
        ...
        <h1 class="page-title mb-0">Editar Transação</h1>
    </div>
